   ...
   */
	...
	*/
	static private function _parse($HTML) {
		...
		preg_match_all('/<[\s]*meta[\s]*(name|property)="?' . '([^>"]*)"?[\s]*' . 'content="?([^>"]*)"?[\s]*[\/]?[\s]*>/si', $HTML, $rawTags, PREG_PATTERN_ORDER);
		//print_r($rawTags);exit;

		if(!empty($rawTags)) {
			$multiValueTags = array_unique(array_diff_assoc($rawTags[2], array_unique($rawTags[2])));

			for($i=0; $i < sizeof($rawTags[2]); $i++) {
